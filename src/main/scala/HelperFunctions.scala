...
import java.io.{ObjectInputStream, OutputStreamWriter}
import java.net.URI
...
object HelperFunctions {
  ...
  val logger: Logger = CreateLogger(classOf[HelperFunctions.type])
  ...
  //  lazy val originalGraph: Option[NetGraph] = NetGraph.load(fileName = originalGraphFileName, dir = NGSGraphDir)
  //  lazy val perturbedGraph: Option[NetGraph] = NetGraph.load(fileName = perturbedGraphFileName, dir = NGSGraphDir)
  ...
  def serializeShardsAsStrings(nGraphs: List[NetGraph], pGraphs: List[NetGraph], dir: String, fileName: String): Unit = {
    ...
    val writer = {
      if hadoopFS == "local" || hadoopFS == "file:///" then
        new BufferedWriter(new FileWriter(s"$dir$fileName"))
        ...
        new BufferedWriter(new OutputStreamWriter(fs.create(hdfsFilePath)))
      else
    ...
    for (nGraph <- nGraphs) {
      for (pGraph <- pGraphs) {
        ...
        val allNGraphNodesAsString: String = ngsm.nodes().asScala.toList.toString
        val allNGraphEdgesAsString: String = ngsm.edges().asScala.toList.map(ep => (ep, ngsm.edgeValue(ep.source(), ep.target()).get().cost)).toString
        val allPGraphNodesAsString: String = pgsm.nodes().asScala.toList.toString
        val allPGraphEdgesAsString: String = pgsm.edges().asScala.toList.map(ep => (ep, pgsm.edgeValue(ep.source(), ep.target()).get().cost)).toString
  ...
  def stringToNodeObject(strObj: String): NodeObject = {
    ...
    if (nodeFields.length != 9) {
      throw new Exception(s"NodeStr: $strObj doesn't have 9 fields!")
    }
  ...
  case class GraphInfo(allNnodes: List[NodeObject], allNIncidentedges: immutable.Map[NodeObject, immutable.Map[(NodeObject, NodeObject), Double]], allPnodes: List[NodeObject], allPIncidentedges: immutable.Map[NodeObject, immutable.Map[(NodeObject, NodeObject), Double]])

  /*//required to deserializ a shard to the mapper1's map input back to node objects.
  def deserializeStringShard(shard_string: String): (Shard, GraphInfo) = {
    ...
    val allNGraphEdges: immutable.Map[NodeObject, NodeObject] = allNGraphEdgesAsStringList.foldLeft(immutable.Map[NodeObject, NodeObject]())((acc, ne) =>
      acc + {
        val Array(sourceAsStr, targetAsStr) = ne.split("->")
        stringToNodeObject(sourceAsStr) -> stringToNodeObject(targetAsStr)
      }
    )
    val allPGraphEdges: immutable.Map[NodeObject, NodeObject] = allPGraphEdgesAsStringList.foldLeft(immutable.Map[NodeObject, NodeObject]())((acc, ne) =>
      acc + {
        val Array(sourceAsStr, targetAsStr) = ne.split("->")
        stringToNodeObject(sourceAsStr) -> stringToNodeObject(targetAsStr)
      }
    ...
    val NGraphIncidentEdges = mutable.Map[NodeObject, mutable.Map[NodeObject, NodeObject]]()
    allNGraphNodes.foreach { nodeN =>
      NGraphIncidentEdges.getOrElseUpdate(nodeN, mutable.Map.empty[NodeObject, NodeObject]) ++= {
        allNGraphEdges.filter(ne => ne._1 == nodeN || ne._2 == nodeN)
      }
    }

    val PGraphIncidentEdges = mutable.Map[NodeObject, mutable.Map[NodeObject, NodeObject]]()
    allPGraphNodes.foreach { nodeP =>
      PGraphIncidentEdges.getOrElseUpdate(nodeP, mutable.Map.empty[NodeObject, NodeObject]) ++= {
        allPGraphEdges.filter(pe => pe._1 == nodeP || pe._2 == nodeP)
      }
    }

    (Shard(allNGraphNodes, allNGraphParentMap, allPGraphNodes, allPGraphParentMap), GraphInfo(allNGraphNodes, NGraphIncidentEdges.map((e1, e2) => (e1, e2.toMap)).toMap, allPGraphNodes, PGraphIncidentEdges.map((e1, e2) => (e1, e2.toMap)).toMap))
  }*/
  ...
  def deserializeStringShard(shard_string: String): (Shard, GraphInfo) = {
    ...
    val allNGraphNodesAsStringList = """NodeObject\([^)]+\)""".r.findAllIn(allNGraphNodesAsString).toList
    val allNGraphEdgesAsStringList = """\(\<NodeObject\([-?[0-9]*\.?[0-9]*,]+\)\s?->\s?NodeObject\([-?[0-9]*\.?[0-9]*,]+\)\>,\s?[\d.]+\)""".r.findAllIn(allNGraphEdgesAsString).toList
    val allPGraphNodesAsStringList = """NodeObject\([^)]+\)""".r.findAllIn(allPGraphNodesAsString).toList
    val allPGraphEdgesAsStringList = """\(\<NodeObject\([-?[0-9]*\.?[0-9]*,]+\)\s?->\s?NodeObject\([-?[0-9]*\.?[0-9]*,]+\)\>,\s?[\d.]+\)""".r.findAllIn(allPGraphEdgesAsString).toList

    val allNGraphEdges = mutable.Map[(NodeObject, NodeObject), Double]()
    val allPGraphEdges = mutable.Map[(NodeObject, NodeObject), Double]()

    val nodeRegex = """NodeObject\([-?[0-9]*\.?[0-9]*,]+\)""".r

    for (nEdgeStr <- allNGraphEdgesAsStringList) {
      // Apply the regular expression to the input string
      val nodesArr = """NodeObject\([-?[0-9]*\.?[0-9]*,]+\)\s?->\s?NodeObject\([-?[0-9]*\.?[0-9]*,]+\)""".r.findFirstIn(nEdgeStr).get.split("->")
      val nodesTuple = (stringToNodeObject(nodesArr(0)), stringToNodeObject(nodesArr(1)))
      val resultString = nodeRegex.replaceAllIn(nEdgeStr, "")
      val doubleValue = """[\d.]+""".r.findFirstIn(resultString).get.toDouble
      allNGraphEdges += nodesTuple -> doubleValue
    }
    for (pEdgeStr <- allPGraphEdgesAsStringList) {
      // Apply the regular expression to the input string
      val nodesArr = """NodeObject\([-?[0-9]*\.?[0-9]*,]+\)\s?->\s?NodeObject\([-?[0-9]*\.?[0-9]*,]+\)""".r.findFirstIn(pEdgeStr).get.split("->")
      val nodesTuple = (stringToNodeObject(nodesArr(0)), stringToNodeObject(nodesArr(1)))
      val resultString = nodeRegex.replaceAllIn(pEdgeStr, "")
      val doubleValue = """[\d.]+""".r.findFirstIn(resultString).get.toDouble
      allPGraphEdges += nodesTuple -> doubleValue
    }

    val tempNGraphAdjacencyMap = mutable.Map[NodeObject, ListBuffer[NodeObject]]()
    allNGraphEdges.keys.foreach { ep =>
      tempNGraphAdjacencyMap.getOrElseUpdate(ep._2, mutable.ListBuffer.empty) += ep._1
    }
    val allNGraphParentMap: immutable.Map[NodeObject, List[NodeObject]] = tempNGraphAdjacencyMap.foldLeft(immutable.Map[NodeObject, List[NodeObject]]())((acc, m) => acc + (m._1 -> m._2.toList))

    val tempPGraphAdjacencyMap = mutable.Map[NodeObject, ListBuffer[NodeObject]]()
    allPGraphEdges.keys.foreach { ep =>
      tempPGraphAdjacencyMap.getOrElseUpdate(ep._2, mutable.ListBuffer.empty) += ep._1
    }
    val allPGraphParentMap: immutable.Map[NodeObject, List[NodeObject]] = tempPGraphAdjacencyMap.foldLeft(immutable.Map[NodeObject, List[NodeObject]]())((acc, m) => acc + (m._1 -> m._2.toList))
    ...
    val NGraphIncidentEdges = mutable.Map[NodeObject, mutable.Map[(NodeObject, NodeObject), Double]]()
    allNGraphNodes.foreach { nodeN =>
      NGraphIncidentEdges.getOrElseUpdate(nodeN, mutable.Map.empty[(NodeObject, NodeObject), Double]) ++= {
        allNGraphEdges.filter(e => e._1._1 == nodeN || e._1._2 == nodeN)
      }
    }

    val PGraphIncidentEdges = mutable.Map[NodeObject, mutable.Map[(NodeObject, NodeObject), Double]]()
    allPGraphNodes.foreach { nodeP =>
      PGraphIncidentEdges.getOrElseUpdate(nodeP, mutable.Map.empty[(NodeObject, NodeObject), Double]) ++= {
        allPGraphEdges.filter(e => e._1._1 == nodeP || e._1._2 == nodeP)
      }
    }
  ...
  def createYamlFileInLocal(dir: String, yamlFileName: String): Unit = {
    ...
    if (!yamlFile.exists) {
      ...
      logger.info(s"Yaml File $yamlFileName already exists!")
    }
  }
  ...
  def readNodeMatchHDFSFile(reducerOutputFilePath: String): immutable.Map[NodeObject, NodeObject] = {
    ...
    val fs = {
      if hadoopFS.startsWith("hdfs://localhost") || hadoopFS.startsWith("s3://") then
  ...
  def decipherNodes(reducer1OutputFilePath: String, newYamlFileDir: String, newYamlFileName: String): NodePerturbationInfo = {

    val graphInfo = deserializeAllNodesEdges(dir = mapReduce1Dir, fileName = graphInfoFileName)._2
    ...
    val PGNodes = graphInfo.allPnodes
    ...
    val removedEdges = removedNodes.foldLeft(List.empty[(NodeObject, NodeObject)])((acc, rn) => {
      val associatedEdges = graphInfo.allNIncidentedges(rn).keys
      acc ++ associatedEdges.map(ae => (ae._1, ae._2))
    ...
    val addedEdges = addedNodes.foldLeft(List.empty[(NodeObject, NodeObject)])((acc, an) => {
      val associatedEdges = graphInfo.allPIncidentedges(an).keys
      acc ++ associatedEdges.map(ae => (ae._1, ae._2))
    }).map((u, v) => u.id.toString.concat(":").concat(v.id.toString)).toList
  ...
  def serializeEdgeInfo(edgesInfoTextFileDir: String, edgesInfoTextFileName: String, nodePerturbationObj: NodePerturbationInfo) = {
    ...
    for (nodeMatch <- otherNodesMap) {
      val nodeU = nodeMatch._1
      val nodeV = nodeMatch._2
      val nodeUEdgesInfo = graphInfo.allNIncidentedges(nodeU).filterNot(ele => nodePerturbationObj.removedNodes.contains(ele._1._1) || nodePerturbationObj.removedNodes.contains(ele._1._2)).toList
      val nodeVEdgesInfo = graphInfo.allPIncidentedges(nodeV).filterNot(ele => nodePerturbationObj.addedNodes.contains(ele._1._1) || nodePerturbationObj.addedNodes.contains(ele._1._2)).toList
  ...
  def deserializeEdgeInfo(shard_string: String): EdgesShard = {
    ...
    for (nodeUEdgeStr <- nodeUEdgesStr) {
      // Apply the regular expression to the input string
      val nodesArr = """NodeObject\([-?[0-9]*\.?[0-9]*,]+\)""".r.findAllIn(nodeUEdgeStr.toString()).toArray
      val nodesTuple = (stringToNodeObject(nodesArr(0)), stringToNodeObject(nodesArr(1)))
    ...
    for (nodeVEdgeStr <- nodeVEdgesStr) {
      // Apply the regular expression to the input string
      val nodesArr = """NodeObject\([-?[0-9]*\.?[0-9]*,]+\)""".r.findAllIn(nodeVEdgeStr.toString()).toArray
      val nodesTuple = (stringToNodeObject(nodesArr(0)), stringToNodeObject(nodesArr(1)))
  ...
  def serializeAllNodesEdges(NGraph: NetGraph, PGraph: NetGraph, dir: String, fileName: String): Unit = {
    val allNGraphNodesAsString: String = NGraph.sm.nodes().asScala.toList.toString()
    val allNGraphEdgesAsString: String = NGraph.sm.edges().asScala.toList.map(ep => (ep, NGraph.sm.edgeValue(ep.source(), ep.target()).get().cost)).toString
    val allPGraphNodesAsString: String = PGraph.sm.nodes().asScala.toList.toString()
    val allPGraphEdgesAsString: String = PGraph.sm.edges().asScala.toList.map(ep => (ep, PGraph.sm.edgeValue(ep.source(), ep.target()).get().cost)).toString
    ...
    if hadoopFS == "local" || hadoopFS == "file:///" then
      ...
      val outputPath = new Path(s"$dir$fileName")
  ...
  def deserializeAllNodesEdges(dir: String, fileName: String): (Shard, GraphInfo) = {
    val filePath = s"$dir$fileName"
    if hadoopFS.startsWith("hdfs://") || hadoopFS.startsWith("s3://") then
      val conf = new org.apache.hadoop.conf.Configuration()
      val fs = FileSystem.get(new URI(hadoopFS), conf)
      ...
      if !fs.exists(hdfsFilePath) then
        ...
        val content = source.mkString
        source.close()
        deserializeStringShard(content)
    else if hadoopFS == "local" || hadoopFS == "file:///" then
  ...
  def load_graph(fileName: String, dir: String, FS: String): Option[NetGraph] = {
    ...
    else if FS.startsWith("hdfs://") || FS.startsWith("s3://") then
      val conf = new org.apache.hadoop.conf.Configuration()
      val fs = FileSystem.get(URI(hadoopFS), conf)
